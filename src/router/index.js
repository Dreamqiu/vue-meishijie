...
// 路由懒加载，按需加载，访问路径的时候才会加载，不访问，不加载
...
router.beforeEach(async (to, from, next) => {
  const token = localStorage.getItem('token'); //获取指定token本地存储的值,在Login.vue页面中有将数据data.data.token值存储到token字段
  const isLogin = !!token;//转成布尔值
  console.log(to);
