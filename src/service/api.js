...
// 可以使用axios请求，也可以自行编写一个模块封装请求的方法，下面为自行封装的模块请求
class HttpRequest {
  constructor(options){
    this.defaults = {
      baseUrl: ''
  ...
  //interceptors方法用于拦截请求和响应的
  interceptors(install){
    //拦截请求，给请求的数据或者头信息添加一些数据
    install.interceptors.request.use(
      config => {
        //token 用于后端识别身份，主要在登录请求页面的时候需要到识别登录身份
        let token = localStorage.getItem('token');
        if (token) {  // 客户端向服务端请求时，判断是否存在token，如果存在的话，则每个http header都加上token
  ...
  request(options){
    ...
    const instance = axios.create(options)  //创建一个axios的子实例instance，可以创建多个，每个互相不影响
    this.interceptors(instance); //调用实例的方法
    return instance
  }
}
...
const http = request.request();   //请求的实例，即axios的实例

// 获取banner数据，返回的是一个promise
