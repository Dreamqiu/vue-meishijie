  ...
  <div class="home">
    ...
    <el-carousel :interval="5000" type="card" height="300px">
      <el-carousel-item v-for="item in banners" :key="item._id">
        <!--query:{menuId:item._id}图片点进去之后的详情页面的地址栏信息：/detail?menuId=5d83bfba2f7cb93a4009cf98-->
    ...
    <div>
      <h2>内容精选</h2>
      <!-- :info='info' waterfall是自行封装的组件库，也可以使用封装好的  -->
      <waterfall ref="waterfall" @view="loadingMenuHandle">
        <!--:info="menuList" 将menuList的数值传入到子组件中menu-card，子组件中props下的info接收-->
...
export default {
  ...
  mounted(){
    ...
    getMenus({page: this.page}).then((data) => {
      ...
      this.pages = Math.ceil(data.data.total/data.data.page_size);  //拿到可刷新的总共多少页
    })
  },
  methods:{
    loadingMenuHandle(){
      // console.log('在外监听的，已到可视区')
      this.page++; //到达可视区后页面加载下一页
      if(this.page > this.pages){  ///判断是否加载完全部的页面，加载完后不需要再加载，isLoading设为false
